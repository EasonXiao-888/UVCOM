...
from uvcom.span_utils import generalized_temporal_iou, span_cxw_to_xx
...
from uvcom.misc import accuracy
import numpy as np
...
def sampling(sim_intra, src_txt, src_txt_mask, epoch):
    ...
    if epoch > 0:
        ...
        src_txt_neg_mask = []
        for i in range(b):
            current_video_sim = sim_intra[i]
            current_video_sim = current_video_sim.sigmoid()
            _, idx = torch.sort(current_video_sim, descending=False)
            if idx[0] == b:
                select_id = idx[1]
            else:
                select_id = idx[0]
            src_txt_neg.append(src_txt[select_id])
            src_txt_neg_mask.append(src_txt_mask[select_id])
        src_txt_neg = torch.stack(src_txt_neg, dim=0)
        src_txt_neg_mask = torch.stack(src_txt_neg_mask, dim=0)
    else:
        src_txt_neg = torch.cat([src_txt[1:], src_txt[0:1]], dim=0)
        src_txt_neg_mask = torch.cat([src_txt_mask[1:], src_txt_mask[0:1]], dim=0)
    return src_txt_neg, src_txt_neg_mask
...
class UVCOM(nn.Module):
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, epoch=None, src_aud=None, src_aud_mask=None):
        ...
        ### Neg Pairs ###
        # import pdb;pdb.set_trace()
        if epoch is not None and epoch >= self.negative_choose_epoch:
            src_txt_neg = src_txt[sim["negative_idx"]]
            src_txt_mask_neg = src_txt_mask[sim["negative_idx"]]
        else:
            src_txt_neg = torch.cat([src_txt[1:], src_txt[0:1]], dim=0)
            src_txt_mask_neg = torch.cat([src_txt_mask[1:], src_txt_mask[0:1]], dim=0)
        # src_txt_neg, src_txt_mask_neg = src_txt[sim["negative_idx"]], src_txt_mask[sim["negative_idx"]]
...
class SetCriterion(nn.Module):
    ...
    def get_sim_loss(self,outputs,targets,indices):
        ...
        # loss_inter = loss_inter * eos_coef_inter
        loss_inter_T = loss_inter_T.mean(0).sum()
        loss_inter = loss_inter.mean(0).sum()
        loss_inter = (loss_inter + loss_inter_T) / 2
        # loss_inter = 0.1 * loss_inter + loss_inter_T
        ...
        for idx,data in enumerate(targets['span_labels']):
            ...
            # # # import pdb;pdb.set_trace()
            #     gt_clip_intra[idx,int(clip_id[0]):int(clip_id[1])] = 1
        sim_clip_intra = sim_clip_intra.log_softmax(dim=-1)
