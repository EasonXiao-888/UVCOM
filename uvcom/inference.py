...
from uvcom.model import build_model
from uvcom.span_utils import span_cxw_to_xx
from uvcom.start_end_dataset import StartEndDataset, start_end_collate, prepare_batch_inputs
...
def setup_model(opt):
    ...
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=opt.lr_drop, gamma=opt.lr_gamma, verbose=True)
    print("lr_drop:",opt.lr_drop)

    if opt.resume is not None:
        logger.info(f"Load checkpoint from {opt.resume}")
        checkpoint = torch.load(opt.resume, map_location="cpu")
        ckpt_for_ddp = {}
        for i,idx in enumerate(checkpoint["model"]):
            ckpt_for_ddp[idx[7:]] = checkpoint["model"][idx]
        model.load_state_dict(checkpoint["model"])
        # model.load_state_dict(ckpt_for_ddp)
        if opt.resume_all:
